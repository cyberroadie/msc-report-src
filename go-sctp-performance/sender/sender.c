...
static void settings_init(void) {
  ...
  settings.bufsize = 1400;
  settings.verbose= false;
}

void sender(char *host, int port, char *message, long count, int number_of_threads) {
  ...
  for(int i = 0; i < number_of_threads; i++) {
    ...
    printf("%i threads left\n", number_of_threads - i - 1);
  }
}
...
static void usage(void) {
  printf("-i <ip address/hostname>   server to connect to (default: localhost)\n"
      ...
      "-b <size>                     message buffer size (default 1400)\n"
...
int main(int argc, char **argv) {
  ...
  sender(settings.host, settings.port, settings.message, settings.count, settings.no_of_threads);
}
